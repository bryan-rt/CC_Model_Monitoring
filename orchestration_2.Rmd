...
Pull the current quarter's scorecard and application data from Supabase and write it to the local cache, if not already present, as a gzipped CSVs to `data/`.
...
if(length(paths) != 3){
  purrr::walk(seq(cohort_date, cohort_date %m+% months(2), 'month'),
                              ...
                              get_cc_scorecard_data(performance_window = model_input_window,
                                                          write = T)
                              })
}
...
if(length(paths) != 3){
  purrr::walk(seq(cohort_date, cohort_date %m+% months(2), 'month'),
                              ...
                              get_apps_data(performance_window = model_input_window,
                                                          write = T)
                              })
}
```
...
As PSI evealutes the shift in the input population, CSI evaluates candidate drivers, not a decomposition: CSI and PSI are independent calculations on different variables. Attributing a specific share of the PSI shift to a given feature would require the scorecard weights and a sensitivity analysis, which is outside the scope of this quarterly review.
...
# comparisons and will occasionally show a gap. 
