...
Primary orchestrator: `orchestration_2.Rmd` (875 lines, 15 chunks).
Validated frontier: line 875 (EOF) — VALIDATED: has run
...
CSI (orchestration_2.Rmd:386-602) is fully built: pulls features, joins to PSI
...
per feature + ci tab) and `csi_summary` (6 segments x 5 features).
  (anchor: :386 = CSI chunk opening fence `\`\`\`{r}`)
  (anchor: :252 = `source(here::here("R/compute_si.R"))` in PSI chunk)

KS (orchestration_2.Rmd:630-822) is fully built: pulls 12-month-lagged
performance cohort, joins to apps+scorecard for score and segment,
...
`ks_quarterly.xlsx` (2 tabs: ks_comparison, decile_rates).
  (anchor: :630 = KS chunk opening fence `\`\`\`{r}`)
  (anchor: :632 = `source(here::here("R/compute_ks.R"))` in KS chunk)
...
tier_certain (FALSE when CI spans a threshold).
  (anchor: :303 = PSI bootstrap)
  (anchor: :508 = CSI bootstrap)
  (anchor: :751 = KS bootstrap)
  (anchor: :795 = Wilson CI for decile bad rates)
...
12 months prior (perf_date at :636). `R/write_metrics_cache.R` writes,
...
Report visuals (D23): `R/report_visuals.R` contains 8 functions for all charts
and tables. PSI trend table (flextable) + trend chart (faceted ggplot), CSI
heatmap + conditional drilldown bars (capped at 3 flagged segments), KS
comparison chart + change table, rank-ordering chart (faceted, free y), and
...
during docx assembly.
  (anchor: :374 = PSI visuals chunk)
  (anchor: :611 = CSI visuals chunk)
  (anchor: :826 = KS visuals chunk)
  (anchor: :836 = Rank ordering chunk)
  (anchor: :871 = Executive summary chunk)
...
| What | Where |
|---|---|
| Decisions (D1-D24) | `.claude/docs/decisions.md` |
