...
| `pull_performance.R` | CREATED | Pull performance data (3 cols from `performance` table, D20). Approved apps only, 12-month-lagged cohort. | `get_performance_data()` | `orchestration_2.Rmd:644` (anchor: `get_performance_data` call in KS cache-miss branch) |
| `compute_ks.R` | CREATED | Shared KS computation helper (D20). Computes KS statistic and decile bad rates per segment. Deciles re-derived per cohort (not frozen). | `compute_ks_stats()` | `orchestration_2.Rmd:627` (anchor: `source` in KS chunk), `R/build_ks_baseline.R` |
...
| `ks_baseline.R` | CREATED | Frozen development KS baseline (D20). Literal R source — diffable, greppable, committed. Generated by `build_ks_baseline.R`. | `ks_baseline` (list) | `orchestration_2.Rmd:628` (anchor: `source` in KS chunk) |
