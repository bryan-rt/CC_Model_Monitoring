...
and `ks_baseline.R` are called before the validated frontier (line 800;
...
| `compute_si.R` | CREATED | Shared stability index kernel for PSI and CSI (D19). Computes percent_dev/current, K-L divergence, epsilon floor, Total rows, summary. Asserts sum(percent_dev)==1 per group. | `compute_stability_index(joined_df, epsilon)` | `orchestration_2.Rmd:268` (PSI chunk), `orchestration_2.Rmd:390` (CSI chunk, via prior source) |
| `pull_features.R` | CREATED | Pull feature data (7 cols from `features` table, D17) | `get_features_data()` | `orchestration_2.Rmd:406` (anchor: `get_features_data` call in CSI cache-miss branch) |
...
| `build_dev_population.R` | CREATED | Generate development population reference (120 rows, frozen vigintile bins, D15) | `build_dev_population(output_path)` | `orchestration_2.Rmd:269` (anchor: `source` + `build_dev_population` call in PSI chunk) |
...
| `pull_performance.R` | CREATED | Pull performance data (3 cols from `performance` table, D20). Approved apps only, 12-month-lagged cohort. | `get_performance_data()` | `orchestration_2.Rmd:629` (anchor: `get_performance_data` call in KS cache-miss branch) |
| `compute_ks.R` | CREATED | Shared KS computation helper (D20). Computes KS statistic and decile bad rates per segment. Deciles re-derived per cohort (not frozen). | `compute_ks_stats()` | `orchestration_2.Rmd:612` (anchor: `source` in KS chunk), `R/build_ks_baseline.R` |
...
| `ks_baseline.R` | CREATED | Frozen development KS baseline (D20). Literal R source — diffable, greppable, committed. Generated by `build_ks_baseline.R`. | `ks_baseline` (list) | `orchestration_2.Rmd:613` (anchor: `source` in KS chunk) |
| `wilson_ci.R` | CREATED | Vectorized Wilson score CI for binomial proportions (D21). Survives zero counts. | `wilson_ci(k, n, conf)` | `orchestration_2.Rmd:270` (anchor: `source` in PSI chunk) |
| `bootstrap_ci.R` | CREATED | Stratified percentile bootstrap for PSI/CSI/KS (D21). Discovers group set from unresampled data. Own RNG stream (seed+3000L). | `bootstrap_ci(data, group_col, stat_fn, B, conf, seed)` | `orchestration_2.Rmd:271` (anchor: `source` in PSI chunk) |
